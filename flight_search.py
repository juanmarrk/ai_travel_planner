def main(source_location, destination_location, budget):

    import json
    import os
    import csv

    fl = './db/Budget_Flights_India.csv'
    ...
    for row in csv.DictReader(open(fl)):
        ...
        if row['From'] == source_location and row['To'] == destination_location and float(row['Price (INR)']) <= float(budget):
            db.append(row)
